 ...
 * "shapes" example.
 *
 * @author: Bruce Quig Michael K�lling Brian Dahlem
 * @version 2018.11.26
 */
public class Canvas {
        ...
        popMenu = new JPopupMenu() {
            {
                ...
                saveVid = new JMenuItem("Save as video...");
                saveVid.addActionListener(e->saveVid());
                add(savePic);
                add(saveVid);
            }
        };
     ...
     */
    public void redraw() {
        // If a video is not being recorded
        ...
        // If there is a video being recorded
        else {
            ...
            try {
                ...
                String sec = String.format("%.1f sec", (double)recFrames / FPS);
                timeLabel.setText(sec);
            }
            catch (IOException e) {
                JOptionPane.showMessageDialog(frame, "Error", "Could not write video frame", JOptionPane.ERROR_MESSAGE);
                stopRecording();
            }
            catch (IllegalStateException e) {
                // If the encoder was already stopped...
                stopRecording();
            }
            
            // Show the drawing on the canvas
            Graphics gc =  bs.getDrawGraphics();
    ...
    private void saveVid() {
        File file = newFile("mp4", "MP4 Videos");
        if (file == null) {
            return;
        }

        recWindow = new JDialog() {
            JButton stopButton;

            {                
                setTitle("Recording MP4");
        ...
        try {
            videoEncoder = AWTSequenceEncoder.createSequenceEncoder(file, FPS);

            recWindow.setVisible(true);
        }
        catch (IOException e) {
            JOptionPane.showMessageDialog(frame, "Error", "Could not create video", JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    public void stopRecording() {
        try {
            ...
            JOptionPane.showMessageDialog(frame, "Error", "Could not write video file", JOptionPane.ERROR_MESSAGE);
        }
    ...
    public static void makeRecording(double seconds) {
        Canvas c = Canvas.getCanvas();
        ...
        while ((double)c.recFrames / FPS < seconds) {
            pic.update();
        }
        c.stopRecording();
